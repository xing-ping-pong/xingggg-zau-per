...
// POST /api/admin/seed-pages - Seed initial pages (admin only)
  ...
  try {
    ...
    for (const pageData of pages) {
      try {
        ...
        createdCount++
        results.push({ slug: pageData.slug, status: 'created', message: 'Successfully created' })
      } catch (error) {
        console.error(`Error creating page ${pageData.slug}:`, error)
        hadErrors = true
        results.push({ slug: pageData.slug, status: 'error', message: error.message })
      }
    }
    ...
    if (hadErrors) {
      return NextResponse.json({
        success: false,
        message: `Pages seeding completed with errors. Created: ${createdCount}, Skipped: ${skippedCount}`,
        data: {
    ...
    return NextResponse.json({
      success: true,
      message: `Pages seeding completed. Created: ${createdCount}, Skipped: ${skippedCount}`,
      data: {
        created: createdCount,
        skipped: skippedCount,
        results
    ...
    return NextResponse.json(
      { success: false, message: 'Failed to seed pages' },
      { status: 500 }
    )
  }
}
